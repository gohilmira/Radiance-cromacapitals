...
class Team extends CI_Controller{
    ...
    public function get_tree(){
     ...
     public function level_view(){
        $this->show->user_panel('team_level_view');
    }
   ...
   /* public function team_generation(){
    ...
    public function team_matrix_generation(){
        $pool='pool1';
         if(isset($_REQUEST['node'])){
			...
			$user_id=$this->session->userdata('user_id');
            $get_node_id=$this->conn->runQuery('*','pool',"u_id='$user_id' and pool_id='$select_pool' order by id desc");
            $node_id=$get_node_id ? $get_node_id[0]->id:false;
            ...
            $select_pool=1;	
			$this->session->set_userdata("user_selected_pool",$select_pool);
            $get_node_id=$this->conn->runQuery('*','pool',"u_id='$user_id' and pool_id='1' order by id desc");
            $node_id=$get_node_id ? $get_node_id[0]->id:false;
            $data['node_id']=$node_id;//$this->session->userdata('user_id');
        }
    ...
    public function offers(){
        $this->show->user_panel('team_offers');
    }
