    ...
    public function add_account(){ 
       ...
       if (!empty($delete)) {
        ...
        if ($check_user_account) {
            $accnts = json_decode($check_user_account[0]->accounts, true) ?: [];
            unset($accnts[$delete - 1]);
            $update['accounts'] = json_encode($accnts);
            $this->db->where('u_code', $u_code)->update('user_payment_methods', $update);
        }
        redirect(base_url(uri_string()));
    }
